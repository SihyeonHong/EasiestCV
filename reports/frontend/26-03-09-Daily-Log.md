...
### 2-1. 이메일 중복 확인 모달 UI 적용 완료

- 디자이너가 확정한 디자인을 `DuplicateEmailAlertDialog.tsx`에 반영 완료했습니다.
- 계정 선택 영역에 테두리를 추가하고, 항목 선택 시 '하이라이트 처리' 및 아이콘 추가를 적용했습니다.
- 하단에 "비밀번호를 잊으셨나요?" 토글 영역과 '비밀번호 초기화' 폼을 추가하여 `useResetPassword` 훅과 연동했습니다.

### 2-2. 다이얼로그 닫힘 오류 및 상태 변경 복구

- 버튼 디자인 변경 과정에서 유실된 다이얼로그 자동 닫힘 동작을 복구했습니다.
- "다른 계정 만들기" 클릭 시 모달이 닫히면서 이메일 확인 상태가 "확인 완료"로 정상 변경되도록 `onClick` 이벤트를 수정했습니다.
