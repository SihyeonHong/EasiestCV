...
- Constraint: 모든 테스트 코드의 설명(describe, it)과 주석은 반드시 "한글"로 작성해야 합니다. 터미널 실행 등 사용자 승인이 필요할 경우, 하던 일을 중단하고 해당 명령어의 목적과 실행 이유를 설명하며 결재를 요청하세요. 코드 작성 시 `.cursor/rules` 디렉토리 내의 규칙을 엄격히 준수해야 하며, 작업 완료 전 반드시 `npm run lint`를 실행하여 린트 에러가 없음을 확인해야 합니다.
