...
class CustomCodeBlock extends BlockEmbed implements QuillBlot {
  declare domNode: HTMLPreElement; // pre 태그
...
export default function AdminEditor({ userid, tid }: Props) {
  ...
  // ------------------ 코드블럭 Tab/Shift+Enter 처리 ------------------
  useEffect(() => {
    if (!quill) return;

    type Range = { index: number; length: number }; // Quill.RangeStatic 타입 대신 직접 정의
